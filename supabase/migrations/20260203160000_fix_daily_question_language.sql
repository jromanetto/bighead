-- Fix: Add language parameter to get_or_create_daily_question function
-- This allows the app to request questions in the user's preferred language
...
BEGIN
    ...
    -- If still no question (language has no questions), fallback to English
    IF selected_question.id IS NULL AND p_language != 'en' THEN
        SELECT q.* INTO selected_question
        FROM questions q
        WHERE q.language = 'en'
        AND q.is_active = true
    ...
    RETURN QUERY
    SELECT
        (SELECT dq.id FROM daily_questions dq WHERE dq.date = target_date AND dq.question_id = selected_question.id),
        target_date,
        selected_question.id,
        final_question_text,
        selected_question.category,
