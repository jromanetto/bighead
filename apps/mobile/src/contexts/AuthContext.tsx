...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateAvatar = useCallback(async (avatarUrl: string) => {
    ...
    try {
      // Always try UPDATE first (profile may exist in DB even if state.profile is null)
      ...
      if (updated) {
        setState(prev => ({ ...prev, profile: updated as UserProfile }));
        return;
      }
      ...
      const { data, error } = await (supabase
        ...
        .upsert({
          id: state.user.id,
          username: state.user.email?.split("@")[0] || `Player_${Date.now()}`,
